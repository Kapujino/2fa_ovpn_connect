...
if [ -z `nmcli -g GENERAL.STATE con show $VPN_CON_NAME` ]; then
    ...
    PW=$FIX_PW$AUTH_CODE
    notify-send 'Connecting VPN' 'NetworkManager should establish the VPN connection' --icon=dialog-information
    ...
    notify-send 'Disconnecting VPN' 'NetworkManager should disconnect the VPN connection' --icon=dialog-information
else
